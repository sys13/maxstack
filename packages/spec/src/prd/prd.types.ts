 ...
 *
 * Lifted near-verbatim from the autofactory generation (the call is logged in
 * the project's lift-vs-reimplement record): the source is already
 * stack-current Zod-4-era TypeScript and
 * drops in cleanly. Adapted only to house style (Biome: tabs, single quotes, no
 * semicolons) and to `import type` for verbatimModuleSyntax. Phase 1 / task 7
 * grows this into the one-system spec (provenance flags + decision ledger +
 * typed spec-ops with branded-ID cross-references); this file is the v3 core.
 *
