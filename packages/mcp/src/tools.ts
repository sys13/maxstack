			...
			inputSchema: {
				...
				properties: {
					...
					ops: {
						...
						description:
							'With section "ops": the op names whose full arg JSON Schema you want, e.g. ["page.addPage","data.addField"]. Ask for the handful you are about to use — every schema at once is ~113k characters and hosts refuse a response that size. Omit it to get every op\'s name, layer and summary without the schemas.',
					},
				},
			},
		},
 ...
 *
 * Every schema at once is ~113k characters, and the reference host does not
...
function opsSection(names: unknown): unknown {
	...
	if (wanted.length === 0)
		return {
			...
			argSchemas:
				'OMITTED — every schema at once is ~113k characters and hosts refuse a payload that size. Ask for the ones you need: query_spec {section:"ops", ops:["page.addPage","data.addField"]}.',
		}
