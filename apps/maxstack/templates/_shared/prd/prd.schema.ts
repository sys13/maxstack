...
// Prefix guards mirror the template-literal types in prd.types.ts.
const idWithPrefix = (prefix: string) =>
  z.string().regex(new RegExp(`^${prefix}-`), `expected id prefixed "${prefix}-"`);

/**
 * Validate a PRD object. Returns the same object typed as PRD on success;
 * throws an AggregateError-style Error listing every problem on failure.
 */
export function validatePRD(prd: PRD): PRD {
  ...
  const phaseIds = new Set(prd.roadmap.phases.map((p) => p.id));
  const activityIds = new Set(prd.discovery.activities.map((a) => a.id));
  const assumptionIds = new Set(prd.assumptions.map((a) => a.id));
  const goalIds = new Set(
    [...prd.goals.businessGoals, ...prd.goals.userGoals].map((g) => g.id),
  );
  const eventIds = new Set(prd.execution.analyticsEvents.map((e) => e.id));
  ...
  isoDate.safeParse(prd.meta.lastUpdated).success ||
  ...
  for (const a of prd.assumptions) {
    ...
    ref(activityIds, a.validatedByActivityId, `assumption ${a.id}.validatedByActivityId`);
  ...
  for (const r of prd.risks) {
    ...
    ref(activityIds, r.validatedByActivityId, `risk ${r.id}.validatedByActivityId`);
  ...
  for (const bucket of [
    ...
    for (const s of bucket)
      ref(reqIds, s.realizedByRequirementId, `scope ${s.id}.realizedByRequirementId`);
  ...
  if (errors.length)
    throw new Error(`PRD validation failed (${errors.length}):\n- ${errors.join("\n- ")}`);
  return prd;
}
